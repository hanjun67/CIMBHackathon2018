...
updater = Updater(token='726876223:AAGHnwp79A-7grj4OskIXCR4ZYT55kaP-FE')
dispatcher = updater.dispatcher

logging.basicConfig(format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=logging.INFO)

def start(bot, update):
    bot.send_message(chat_id=update.message.chat_id, text="I'm a bot, I can create QR codes!") # Welcome message
...
def op(bot, update):
    ...
    createqr(text)
    bot.send_photo(chat_id=update.message.chat_id, photo=open('qrcode.png', 'rb'))
